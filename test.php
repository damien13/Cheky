	...
	public function check()
	{
		...
		foreach ($users AS $user) {
			...
			foreach ($alerts AS $i => $alert) {
				...
				if (!$content = $this->_httpClient->request($alert->url)) { //todo : recuperer le contenu ici
...
//var_dump($ads); die();
				...
				foreach ($ads AS $ad) {
					...
					$adComplete = $parser->processAd($this->_httpClient->request($ad->getLink()),PHP_URL_SCHEME);
					$ad->setDescription($adComplete->getDescription());
					$time = $ad->getDate();
					$id = $ad->getId();
